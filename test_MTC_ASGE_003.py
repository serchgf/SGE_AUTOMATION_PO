...
class SgePoUnittest(unittest.TestCase):
    """
    se necesita una po valida sin DSO generada por test_MTC_ASGE_001
    guardadas en /facturas_generadas/confirmadas_validas/
    """
    #@file_data("./Config/TestDataset/dataSet_ddt_po.json")
    def test_PurchaseOrder_Creacion_Nacional_Fact_Valida(self):

        print("Test Case 03 PurchaseOrder_Creacion_Nacional_Fact_Valida")
        ...
        if conectado:
            ...
            print("3 Ingresar archivo de factura INV en la carpeta de la ruta. subir_archivo")
            num_factura_valida, inv_factura_valida, archivo_seleccionado = sge_funct.subir_archivo_sftp2(files.inv_confirmadas_validas_folder)
            ...
            sge_funct.send_text(num_factura_valida)
            ...
            if sge_funct.validar_po_status2(img_status, nombre_status):
                ...
                if sge_funct.re_validate_status(img_status):
                    ...
                    directorio_origen = files.inv_confirmadas_validas_folder
                    directorio_destino = files.inv_en_transito_folder
                    sge_funct.mover_facturas(num_factura_valida, directorio_origen, directorio_destino)
                else:
