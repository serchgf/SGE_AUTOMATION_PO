...
class SgePoUnittest(unittest.TestCase):
    ...
    def test_PurchaseOrder_Creacion_Nacional_SINDSO(self):
        ...
        if conectado:
            ...
            sge_funct.press_enter()
            print("12 Seleccionar <<OPCION MENU>> 7")
            sge_funct.send_text("7")
            print("13 Ingresar el nombre del <<Proveedor Internacional >> <<nombreproveedorinternacional>>")
            sge_funct.press_tab()
            sge_funct.send_text("USMEXICO")
            print("14 Teclear input  Enter")
            sge_funct.press_enter()
            print("15 Presionar la combinacion de teclas: Ctrl + F ctrl+f")
            sge_funct.press_hotkeys("ctrl+f")
            print("16 Ingresar el usuario almacen <<USUARIOALMACEN>>")
            ...
            nombre_status = "ACOMODADA PEND DE PA"
            ...
            if sge_funct.validar_po_status2(img_status_confirmada, nombre_status):
                ...
                print("Creando archivos: fac.tura valida y factura invalida")
                #en carpetas de tmp
