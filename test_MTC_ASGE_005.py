...
class SgePoUnittest(unittest.TestCase):
    ...
    def test_PurchaseOrder_AsignacionFactura_Nacional_Cancelacion(self):
        ...
        print("Test Case 04 PurchaseOrder_AsignacionFactura_Nacional_Cancelacion")
        ...
        if conectado:
            ...
            job_status_pend = images.img_status_job_pend
            job_status_correcto = sge_funct.validar_job_status_x(job_status_pend)
            if job_status_correcto:
                ...
                sge_funct.sge_connection(login_data.adm_username, login_data.ip, login_data.adm_password)
                print("15 dirigirse a la <<RUTA>> cd automation")
                sge_funct.send_text("cd automation")
                print("16 Press Enter Enter")
                sge_funct.press_enter()
                print("17 Teclear input principaluat02")
                sge_funct.send_text("principaluat02")
                print("18 Press Enter Enter")
                sge_funct.press_enter()
                print("19 tiempo fuera 2 segundos")
                sge_funct.segundos_de_espera(2)
                print("20 Seleccionar <<Sucursal>> 12")
                sge_funct.send_text("12")
                print("21 Teclear input  Enter")
                sge_funct.press_enter()
                print("22 Seleccionar <<OPCION MENU>> 2")
                sge_funct.send_text("2")
                print("23 Teclear input  Enter")
                sge_funct.press_enter()
                print("24 Seleccionar <<OPCION MENU>> 4")
                sge_funct.send_text("4")
                print("25 Teclear input  Enter")
                sge_funct.press_enter()
                print("26 Seleccionar <<OPCION MENU>> 10")
                sge_funct.send_text("10")
                print("27 Teclear input  Enter")
                sge_funct.press_enter()
                print("28 Seleccionar <<OPCION MENU>> 6")
                sge_funct.send_text("6")
                print("29 escribir inv_factura_invalida")
                sge_funct.send_text(inv_factura_invalida)
                print("30  ctrl+b")
                sge_funct.press_hotkeys("ctrl+b")
                print("31 escribir usuario adm grejudl")
                sge_funct.send_text("grejudl")
                print("32 Press Enter Enter")
                sge_funct.press_enter()
                print("33 Press Enter Enter")
                sge_funct.press_enter()
                print("34 Press Enter Enter")
                sge_funct.press_enter()
                ...
                sge_funct.send_text("2")
                print("38  Abrir y copiar contenido de archivo <<Arcivo INV-ordenCompra.txt>> pegar_num_po")
                ...
                nombre_status = "CANCELADA COMPRAS"
                ...
                if sge_funct.validar_po_status2(img_status, nombre_status):
                    ...
                    if sge_funct.re_validate_status(img_status):
                        ...
                        directorio_destino = files.inv_canceladas_folder
                ...
                assert False, "Estatus no esperado, deberia ser PEND"
        else:
            print(conectado)
